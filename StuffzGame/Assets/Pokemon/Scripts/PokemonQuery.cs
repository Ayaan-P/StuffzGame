...
public class PokemonQuery
{
    ...
    public bool CanPokemonEvolveAtLevel(int level, Pokemon pokemon)
    {
        var evolutions = GetEvolutionsFor(pokemon);
        ...
        {
            foreach(var evolvedPokemon in evolutions)
            {
               var evolutionDetails =  evolvedPokemon.Species.EvolvesFrom.EvolutionDetails;
                foreach(var evoDetail in evolutionDetails)
                {
                    if(evoDetail.Trigger == EvolutionTrigger.LEVEL_UP)
                    {
                        if(evoDetail.MinLevel != null && level >= evoDetail.MinLevel)
                        {
                            return true;
                        }
                    }
